...
def train_one_epoch(
    ...
    if deepcot:
        if dataparallel:
            model.module.call_mode = 'forward_steps'
        else:
...
def evaluate(model, criterion, data_loader, device, logger, args, epoch, nprocs=4):
    ...
    for (
        ...
        class_h_target_val,
        dec_target,
    ) in tqdm(metric_logger.log_every(data_loader, 50, header)):
        if camera_inputs_val.shape[0] != 128:
            continue
        ...
        if args.model in ['base', 'nystromformer']:
            temporal_dim = 1

            cam_shape = list(camera_inputs.shape)
            window_size = cam_shape[temporal_dim]
            cam_shape[temporal_dim] -= 1
            camera_inputs = torch.cat((torch.zeros(cam_shape, device=camera_inputs.device), camera_inputs), dim=temporal_dim)

            motion_shape = list(motion_inputs.shape)
            motion_shape[temporal_dim] -= 1
            motion_inputs = torch.cat((torch.zeros(motion_shape, device=motion_inputs.device), motion_inputs), dim=temporal_dim)

            for temp_idx in range(window_size):
                cam_window = camera_inputs[:, temp_idx:temp_idx+window_size]
                motion_window = motion_inputs[:, temp_idx:temp_idx+window_size]

                start_time = time.time()
                enc_score_p0 = model(
                    torch.cat((cam_window, motion_window), 2).transpose(1, 2)
                ).squeeze(-1)
                end_time = time.time()
    ...
    if args.distributed:
        ...
        if is_main_process():
            ...
            map, aps, cap, _ = frame_level_map_n_cap(results)
            logger.output_print(
