...
def subtraction_gaussian_kernel_torch(q, k, attention_mask=None):
	...
	gaussian_res = matA_square + matB_square - 2. * (q @ k)

	if attention_mask is not None:
		gaussian_res = gaussian_res - attention_mask
	gaussian_res = torch.clamp(gaussian_res, min=0.0)
	return gaussian_res

def gaussian_attention(q, k, attention_mask = None):
	return torch.exp(-subtraction_gaussian_kernel_torch(q, k, attention_mask=attention_mask) / (2*q.shape[-1]))
...
def multi_head_attention_forward(
	...
	if need_weights:
		...
		attn_output_weights = gaussian_attention(q, k.transpose(-2, -1))
		if attn_mask is not None:
