...
from .gaussian_attention import multi_head_attention_forward as gaussian_multi_head_attention_forward, _is_make_fx_tracing, _check_arg_device, _arg_requires_grad
...
class MultiheadAttentionBase(CoModule, MultiheadAttention):
    ...
    def gaussian_forward(
       ...
       is_causal: bool = False,
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = gaussian_multi_head_attention_forward(
                query,
                key,
                value,
                self.embed_dim,
    ...
    def forward(
        ...
        fwd_function = MultiheadAttention.forward if self.attention_act == 'softmax' else MultiheadAttentionBase.gaussian_forward

        o, attn_output_weights = fwd_function(
        ...
        if prev_state is None:
            prev_state = (
                *self._sdpa_default_state(
                ...
                torch.tensor(-self.sequence_len) if use_stride else torch.tensor(0),
            )
    ...
    def forward_steps(
        ...
        **kwargs,
        ...
        for t in range(T):
            o, tmp_state = MultiheadAttentionBase._forward_step(
                self, query[t], key[t], value[t], tmp_state, use_stride=use_stride,
            )
