...
from .gaussian_attention import gaussian_attention
...
def _gaussian_scaled_dot_product_attention_step(
    prev_state: State,
    q_step: Tensor,  # step input (B, E)
    k_step: Tensor,  # step input (B, E)
    v_step: Tensor,  # step input (B, E)
    ...
    (
        d_mem,  # (B, Nt-1) # TODO: Not used for gaussian
        AV_mem,  # (B, Ns-1, E)
        Q_mem,  # (B, Nt-1, E)
        K_T_mem,  # (B, E, Ns)
        V_mem,  # (B, Ns, E)
    ...
    # Left column attention values
    A_left = gaussian_attention(Q_mem, K_T_mem[:, :, 0].unsqueeze(-1))

    # Right column attention values
    A_right = gaussian_attention(Q_mem, k_step.unsqueeze(-1))
    ...
    # Bottom row attention values
    A_bottom = gaussian_attention(q_step.unsqueeze(1), K_T_mem_new)
...
class RetroactiveMultiheadAttention(MultiheadAttentionBase):
    ...
    def __init__(
        ...
        MultiheadAttentionBase.__init__(
            ...
            _scaled_dot_product_attention_step if attention_act=='softmax' else _gaussian_scaled_dot_product_attention_step,
    ...
    def flops(self, call_mode=None):
        ...
        if call_mode == _callmode("forward"):
            return MultiheadAttentionBase.flops(self)
        else:
            f = 7*n*d + 4*n - 2*d - 2
            return f
    ...
    def mem_costs(self, call_mode=None):
        ...
        if call_mode == _callmode("forward"):
            return MultiheadAttentionBase.mem_costs(self)
        else:
            valley_cost = 4*n*d + n - d - 4
            peak_cost = 5*n*d + 2*n
