...
from .delay import State as DelayState
...
from .multihead_attention import (
    ...
    ContinualNystromMultiheadAttention,
...
__all__ = [
    ...
    "SingleOutputNystromTransformerEncoderLayer",
    "NystromTransformerEncoderLayerFactory",
    "NystromTransformerEncoder",
]

from .multihead_attention.mha_base import MultiheadAttentionBase
...
class MultiheadAttentionModule(MultiheadAttentionBase):
    def __init__(self, embed_dim, num_heads, dropout, bias, batch_first, device, dtype, sequence_len=None, attention_act='softmax'):
        super().__init__(
            embed_dim=embed_dim,
            num_heads=num_heads,
            dropout=dropout,
    ...
    def flops(self, call_mode='forward'):
        if call_mode != 'forward':
            warnings.warn('flops computation only work in forward mode')
    ...
    def mem_costs(self, call_mode='forward'):
        if call_mode != 'forward':
            warnings.warn('mem cost computation only work in forward mode')

        d = self.embed_dim
        n = self.sequence_len

        valley_cost = 3*(n*d - 1)
        peak_cost = n**2 + 4*n*d + 1
        return valley_cost, peak_cost

class NonContinualMultiheadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, bias, batch_first, device, dtype, sequence_len, attention_act='softmax', *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.self_attn = MultiheadAttentionModule(
            embed_dim=embed_dim,
            num_heads=num_heads,
            dropout=dropout,
            bias=bias,
    ...
    def forward(self, query, key=None, value=None):
        ...
        key = key.permute(0, 2, 1) if key else query
        value = value.permute(0, 2, 1) if value else query

        out_attn = self.self_attn(query, key, value)[0]
        return (out_attn + query).permute(0, 2, 1)
...
def StepLocalTransformerEncoderLayer(
    ...
    mha = NonContinualMultiheadAttention(
        embed_dim=d_model,
        num_heads=nhead,
        dropout=dropout,
        bias=True,
...
class NonContinualNystromMultiheadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, bias, batch_first, device, dtype, sequence_len, num_landmarks, batch_size, fixed_landmarks, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.self_attn = NystromMultiheadAttention(
            embed_dim=embed_dim,
            num_heads=num_heads,
            dropout=dropout,
            bias=bias,
...
def StepLocalNystromTransformerEncoderLayer(
    d_model: int,
    nhead: int,
    num_landmarks: int,
    dim_feedforward: int = 2048,
    ...
    mha = NonContinualNystromMultiheadAttention(
        embed_dim=d_model,
        num_heads=nhead,
        dropout=dropout,
        bias=True,
...
def NystromTransformerEncoderLayerFactory(
    ...
    def TransformerEncoderLayer(mha_type: MhaType):
        factory_fn = {
            ...
            MhaType.NYSTROMFORMER: StepLocalNystromTransformerEncoderLayer,
        }[MhaType(mha_type)]
...
class TransformerEncoder(Sequential):
    ...
    def build_from(
        ...
        layer_factory = TransformerEncoderLayerFactory(
            ...
            dtype=tel.linear1.weight.dtype,
            sequence_len=sequence_len,
            attention_act=tel.attenion_act,
        )
        net = TransformerEncoder(
            layer_factory, num_layers=trans_enc.num_layers, norm=trans_enc.norm
        )
