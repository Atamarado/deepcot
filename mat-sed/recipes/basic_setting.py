...
from src.dataset import (StronglyLabeledDataset, WeaklyLabeledDataset, UnlabeledDataset, ConcatDatasetBatchSampler,
                         UrbanDataset)
...
def get_logger(save_folder, need_tensorboard=True, log_level=logging.INFO) -> Logger:
    logger = Logger()
...
def get_datasets(config, encoder, evaluation, test_only, logger, dataset='dcase'):
    if not evaluation:
        if dataset == 'urban':
            devtest_dataset = UrbanDataset('URBAN-PATH', 'test', encoder=encoder, return_name=True)
        else:
            devtest_df = pd.read_csv(config["dataset"]["test_tsv"], sep="\t")
            missing_df = pd.read_csv(
                'DESED-PATH/recipes/dcase2023_task4_baseline/missing_files/missing_files_validation.tsv',
                sep='\t')
            devtest_df = devtest_df[~devtest_df['filename'].isin(missing_df['filename'])]
        ...
        if dataset == 'urban':
            devtest_dataset = UrbanDataset('URBAN-PATH', 'eval', encoder=encoder, return_name=True)
        else:
    ...
    if not test_only:
        if dataset == 'urban':
            ...
            train_dataset = UrbanDataset('URBAN-PATH', 'train', encoder=encoder, return_name=True)
            valid_dataset = UrbanDataset('URBAN-PATH', 'validate', encoder=encoder, return_name=True)
        else:
            ...
            existing_files = [filename for filename in set(train_strong_df['filename']) if os.path.exists(os.path.join(config["dataset"]["strong_folder"], filename))]
            train_strong_df = train_strong_df[train_strong_df['filename'].isin(existing_files)]
            strong_set = StronglyLabeledDataset(tsv_read=train_strong_df,
                                                dataset_dir=config["dataset"]["strong_folder"],
                                                return_name=False,
                                                encoder=encoder)

            weak_df = pd.read_csv(config["dataset"]["weak_tsv"], sep="\t") # All data is available now :)
            ...
            samplers = []
            for x in tot_train_data:
                if len(x) > 0:
                    samplers.append(torch.utils.data.RandomSampler(x))
                else:
                    raise Exception("There is a dataset with 0 samples.")
