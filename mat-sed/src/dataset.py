...
class UrbanDataset(Dataset):
    def __init__(self, path, split, encoder, return_name):
        self.encoder = encoder
        self.return_name = return_name
        self.pad_to = encoder.audio_len * encoder.sr

        self.annotation_path = os.path.join(path, 'annotations', split)
        self.audio_paths = os.path.join(path, 'audio', split)
        self.audio_files = [file for file in os.listdir(self.audio_paths) if file.startswith('soundscape')]
    ...
    def __getitem__(self, idx):
        ...
        annotation_file = os.path.join(self.annotation_path, audio_file.replace('wav', 'txt'))
        annotations = pd.read_csv(annotation_file, sep='\t', names=['onset', 'offset', 'event_label'])

        audio_path = os.path.join(self.audio_paths, audio_file)
        wav, pad_mask = waveform_modification(audio_path, self.pad_to, self.encoder)

        if len(annotations) == 0:  #label size = [frames, nclass]
            label = torch.zeros(self.encoder.n_frames, len(self.encoder.labels)).float()
        else:
            label = self.encoder.encode_strong_df(annotations)
            label = torch.from_numpy(label).float()
        label = label.transpose(0, 1)

        out_args = [wav, label, pad_mask, idx]
        if self.return_name:
            out_args.extend([audio_file, audio_path])
        return out_args

    def get_annotation_df(self, idx_list):
        annotations_list = []
        for idx in idx_list:
            audio_file = self.audio_files[idx]
            ...
            annotation_file = os.path.join(self.annotation_path, audio_file.replace('wav', 'txt'))
            annotations = pd.read_csv(annotation_file, sep='\t', names=['onset', 'offset', 'event_label'])
            annotations['filename'] = audio_file
