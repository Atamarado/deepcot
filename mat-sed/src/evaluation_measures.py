...
def segment_based_evaluation_df(reference, estimated, time_resolution=1.0):
    ...
    segment_based_metric = sed_eval.sound_event.SegmentBasedMetrics(event_label_list=classes,
...
def log_sedeval_metrics(predictions, ground_truth, save_dir=None, return_class_wise=False):
...
def compute_psds_from_scores(
    ...
    # TODO: Change back when we get all the data
    ground_truth = pd.read_csv(ground_truth_file, sep="\t")
    ground_truth = ground_truth[ground_truth['filename'].str.replace('.wav', '').isin(scores.keys())]
    ground_truth.to_csv("ground_truth.tsv", index=False, sep="\t") # TODO: Hotfix to make it work

    audio_durations = pd.read_csv(durations_file, sep="\t")
    audio_durations = audio_durations[audio_durations['filename'].str.replace('.wav', '').isin(scores.keys())]
    audio_durations.to_csv("audio_durations.tsv", index=False, sep="\t")  # TODO: Hotfix to make it work

    psds, single_class_psds, psd_roc, single_class_rocs, *_ = sed_scores_eval.intersection_based.psds(
        scores=scores,
        ground_truth="ground_truth.tsv",
        audio_durations="audio_durations.tsv",
        dtc_threshold=dtc_threshold,
        gtc_threshold=gtc_threshold,
        cttc_threshold=cttc_threshold,
        alpha_ct=alpha_ct,
    ...
    os.remove("ground_truth.tsv")
    os.remove("audio_durations.tsv")
    if save_dir is not None:
        scores_dir = os.path.join(save_dir, "scores")
        sed_scores_eval.io.write_sed_scores(scores, scores_dir)
        filename = (f"PSDS_dtc{dtc_threshold}_gtc{gtc_threshold}_cttc{cttc_threshold}"
