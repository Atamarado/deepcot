...
from src.models.passt.vit_helpers import (
...
class PatchEmbed(nn.Module, co.CoModule):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None,
        ...
        self.proj = co.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def clean_state(self):
        self.proj.clean_state()
    ...
    def forward(self, x):
        ...
        if x_out is None:
            self.proj.stride_index = torch.tensor(9, dtype=torch.int64) # TODO: Hotfix
...
class Attention(co.CoModule, nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., window_size=-1):
    ...
    def forward(self, x, att_mask=None, vectorize_transformers=False):
        B, N, C = x.shape
...
def generate_causal_sliding_masks(S, N, device=None):
    # Create row and column indices
    row_indices = torch.arange(S).view(-1, 1)  # Shape (S, 1)
    col_indices = torch.arange(S).view(1, -1)  # Shape (1, S)

    # Create the bottom-left triangular mask (including diagonal and lower part)
    bottom_left_mask = row_indices >= col_indices + N

    # Create the top-right triangular mask with N-1 elements to the left of the diagonal
    top_right_mask = row_indices < col_indices

    # Get the minus infinite mask
    minf_mask = (bottom_left_mask | top_right_mask).int() * torch.finfo().min
    minf_mask[:N, :N] = 0

    zeros_mask = torch.ones((S, S))
    zeros_mask[:N, :N] = torch.tril(torch.ones((N, N)))

    if device is None:
        return minf_mask, zeros_mask
    else:
        return minf_mask.to(device), zeros_mask.to(device)

class DeepCoTAttention(Attention):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., window_size=-1):
        Attention.__init__(self, dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop)
    ...
    def init_state(self, batch_size, window_size, device=None):
        self.key_mem_t = torch.zeros(
            (batch_size, self.num_heads, self.head_dim, window_size)).to(device)
        self.value_mem = torch.zeros(
            (batch_size, self.num_heads, window_size, self.head_dim)).to(device)
    ...
    def forward(self, x, att_mask=None, vectorize_transformers=True):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        ...
        if att_mask is not None:
            attn.masked_fill_(att_mask, float("-inf"))
        attn = self.apply_causal_softmax(attn)
        attn = self.attn_drop(attn)
    ...
    def _forward_step_attention(self, q_step, k_step, v_step, att_mask=None):
        if att_mask is not None:
            raise Exception('Attention masks are not supported.')

        if len(self.key_mem_t) == 0:
            self.init_state(batch_size=q_step.shape[0], window_size=self.window_size, device=q_step.device)

        # Perform rolling step
        self.key_mem_t = torch.roll(self.key_mem_t, shifts=-k_step.size(2), dims=3)
        self.key_mem_t[:, :, :, -k_step.size(2):] = k_step.transpose(-1, -2)

        self.value_mem = torch.roll(self.value_mem, shifts=-v_step.size(2), dims=2)
        self.value_mem[:, :, -v_step.size(2):] = v_step

        attn = (q_step @ self.key_mem_t) * self.scale
        # if att_mask is not None:
        #     attn.masked_fill_(att_mask, float("-inf"))
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ self.value_mem)
        return x

    def forward_steps(self, x, att_mask=None, vectorize_transformers=False):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 1, 0, 3, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        if len(self.key_mem_t) == 0:
            self.clean_state(always_clean=False)
            self.init_state(batch_size=q.shape[1], window_size=self.window_size, device=q.device)

        if N > 1 and vectorize_transformers:
            q = q.permute(1, 2, 0, 3)
            k = k.permute(1, 2, 0, 3)
            v = v.permute(1, 2, 0, 3)
            x = self._forward_step_attention(q, k, v, att_mask)
        else:
            x = []
            for q_step, k_step, v_step in zip(q, k, v):
                x.append(self._forward_step_attention(q_step.unsqueeze(2), k_step.unsqueeze(2), v_step.unsqueeze(2), att_mask))
            x = torch.cat(x, dim=2).transpose(1, 2)
...
class Block(co.CoModule, nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, continual=False, window_size=-1):
        co.CoModule.__init__(self)
        nn.Module.__init__(self)
        self.norm1 = norm_layer(dim)
        self.continual = continual
        attn_fn = DeepCoTAttention if continual else Attention
        self.attn = attn_fn(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, window_size=window_size)
    ...
    def forward(self, x, att_mask=None, vectorize_transformers=False):
        x = x + self.drop_path(self.attn.forward(self.norm1(x), att_mask))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x

    def forward_steps(self, x, att_mask=None, vectorize_transformers=False):
        x = x + self.drop_path(self.attn.forward_steps(self.norm1(x), att_mask, vectorize_transformers=vectorize_transformers))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x

class PaSST(co.CoModule, nn.Module):
    """
    ...
    def __init__(self, u_patchout=0, s_patchout_t=0, s_patchout_f=0, img_size=(128, 998), patch_size=16, stride=16,
                 ...
                 act_layer=None, weight_init='', continual=False, window_size=-1):
        """
        Args:
            ...
            continual: (bool): Whether to use continual inference or not
        """
        super().__init__()
        ...
        if continual:
            self.time_new_pos_embed = co.RecyclingPositionalEncoding(embed_dim, self.patch_embed.grid_size[1])
        else:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, continual=continual, window_size=window_size)
    ...
    def clean_state(self):
        self.time_new_pos_embed.clean_state()
        self.patch_embed.clean_state()
        for block in self.blocks:
            block.attn.clean_state()
    ...
    def init_weights(self, mode=''):
        ...
        if self.continual:
            trunc_normal_(self.time_new_pos_embed.pe.weight, std=.02)
        else:
    ...
    def forward_features(self, x, vectorize_transformers=False):
        ...
        if self.continual:
            self.time_new_pos_embed.clean_state()
            x = self.time_new_pos_embed(x.permute(0, 2, 1, 3).flatten(0, 1)).unflatten(0, (B_dim, F_dim)).permute(0, 2, 1, 3)
        else:
        ...
        for k,block in enumerate(self.blocks):
            x = block(x, vectorize_transformers=vectorize_transformers)
        ...
        if self.continual or self.dist_token is None:
    ...
    def forward(self, x):
        ...
        #############
        x = self.forward_features(x, vectorize_transformers=x.size(-1) == 1)
        if x is None:
            return None
        output_dict= x[-1]
...
class PaSST_fe(nn.Module):
    ...
    def __init__(self, u_patchout=0, s_patchout_t=0, s_patchout_f=0, img_size=(128, 998), patch_size=16, stride=16,
                 ...
                 num_heads=12, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init='',mean_pool=False,tte=False,fte=False):
        """
        Args:
            u_patchout: Unstructured Patchout integer, number of items to be removed from the final sequence
            s_patchout_t: structured Patchout time integer, number of columns to be removed from the patches grid
        ...
        self.f_dim,self.t_dim = self.patch_embed.grid_size[0],self.patch_embed.grid_size[1]
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.dist_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None
        # PaSST
        # refer to https://arxiv.org/abs/2110.05069 Section 2
        ...
        ###if use time-wise transformer encoder
        self.tte=tte
        if self.tte:
            self.linear_embed = nn.Linear(1,embed_dim)
            self.tte_blocks = nn.Sequential(*[
                Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate,norm_layer=norm_layer, act_layer=act_layer)
                for i in range(2)])
            self.tte_norm = norm_layer(embed_dim)
        self.fte=fte
        if self.fte:
            assert self.tte==True
            self.linear_embed = nn.Linear(1, embed_dim)
            self.fte_blocks = nn.Sequential(*[
                Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, norm_layer=norm_layer, act_layer=act_layer)
                for i in range(2)])
            self.fte_norm = norm_layer(embed_dim)
        self.init_weights(weight_init)
    ...
    def forward_features(self, x):
        ...
        ###
        # Flatten the sequence
        B_dim_afp, E_dim_afp, F_dim_afp, T_dim_afp = x.shape
        x = x.flatten(2).transpose(1, 2)
        # Unstructured Patchout
        if first_RUN: print("X flattened", x.size())
        if self.training and self.u_patchout:
        ...
        x = self.norm(x)
        if self.fte:
            x = x[:, 2:, :]  ##remove old cls token and distill token
            B_, P_, C_ = x.shape
            x = x.reshape(B_, F_dim_afp, T_dim_afp, C_)  ##reshape to B,F,T,C
            x = x.transpose(1,2)  # B,T,F,C
            x = x.reshape(B_*T_dim_afp,F_dim_afp,C_)#BT*F*C
            new_cls_token = self.linear_embed(torch.ones(size=(x.shape[0], 1, 1)).cuda())
            x = torch.cat([new_cls_token, x], dim=1)  # B*T,F+1,C
            x = self.fte_blocks(x)
            x = self.fte_norm(x)
            x = x[:,0,:]#B*T,1,C
            x = x.reshape(B_,T_dim_afp,C_)
            if self.tte:
                new_cls_token_1 = self.linear_embed(torch.ones(size=(x.shape[0],1,1)).cuda())
                x = torch.cat([new_cls_token_1,x],dim=1)#B,T+1,C
                x = self.tte_blocks(x)
                x = self.tte_norm(x)
        else:
            if self.tte:
                ...
                B_,P_,C_=x.shape
                x = x.reshape(B_,F_dim_afp, T_dim_afp,C_)##reshape to B,F,T,C
                x = torch.mean(x,dim=1)#B,T,C
                new_cls_token = self.linear_embed(torch.ones(size=(x.shape[0],1,1)).cuda())
                x = torch.cat([new_cls_token,x],dim=1)#B,T+1,C
                x = self.tte_blocks(x)
                x = self.tte_norm(x)
        if self.dist_token is None:
            if self.tte:
            ...
            if self.tte:
                ...
                return torch.mean(x[:, 2:, :], dim=1),torch.mean(x[:, 2:, :], dim=1)
            else:
                return x[:, 0], x[:, 1]
...
##add by li
def _create_vision_transformer_my(variant, pretrained=False, default_cfg=None, **kwargs):
...
def debug_model(**kwargs):
    pass
def passt_deit_bd_p16_384_fe_mp(pretrained=False, **kwargs):
    """ PaSST pre-trained on AudioSet
    """
    print("\n\n Loading DEIT BASE 384 with frequency enhanced block\n\n")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_my(
        'deit_base_distilled_patch16_384', pretrained=pretrained, distilled=True, **model_kwargs)
    return model
def passt_deit_bd_p16_384_fe_tte(pretrained=False, **kwargs):
    """ PaSST pre-trained on AudioSet
    """
    print("\n\n Loading DEIT BASE 384 with frequency enhanced block\n\n")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_my(
        'deit_base_distilled_patch16_384', pretrained=pretrained, distilled=True, **model_kwargs)
    return model
def passt_deit_bd_p16_384_fe_fte_tte(pretrained=False, **kwargs):
    """ PaSST pre-trained on AudioSet
    """
    print("\n\n Loading DEIT BASE 384 with frequency enhanced block\n\n")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
...
#     ###add by li for debug####
#     elif arch == "debug":
#         model_func= debug_model
#     elif arch == "passt_deit_bd_p16_384_fe_mp":
#         model_func = passt_deit_bd_p16_384_fe_mp
#     elif arch == "passt_deit_bd_p16_384_fe_tte":
#         model_func = passt_deit_bd_p16_384_fe_tte
#     elif arch == "passt_deit_bd_p16_384_fe_fte_tte":
#         model_func = passt_deit_bd_p16_384_fe_fte_tte
#     #####################
#     if model_func is None:
#         raise RuntimeError(f"Unknown model {arch}")
#
#     if model_func in [passt_deit_bd_p16_384_fe_mp]:
...
#                            s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f,mean_pool=True)
#     elif model_func in [passt_deit_bd_p16_384_fe_tte]:
#         model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
#                            img_size=input_size, stride=stride, u_patchout=u_patchout,
#                            s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f, tte=True)
#     elif model_func in [passt_deit_bd_p16_384_fe_fte_tte]:
#         model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
#                            img_size=input_size, stride=stride, u_patchout=u_patchout,
#                            s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f, fte=True,tte=True)
#     else:
#         model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
#                            img_size=input_size, stride=stride, u_patchout=u_patchout,
