...
class PaSST_SED(co.CoModule, nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_transformer_sliding_window = None
        ...
        self.slide_window_model = None

    def prepare_continual(self):
        self.patch_transformer_sliding_window = copy.deepcopy(self.patch_transformer)
        for block in self.patch_transformer_sliding_window.blocks:
            block.attn.window_size = 600 # TODO: Hotfix to set the window correctly
    ...
    def f_pool(self, passt_out_dict):
        ...
        if self.passt_feature_layer == 'frame':
            passt_feature = passt_out_dict['frame']
            ...
            if self.continual:
                feature_list = [passt_out_dict["layer{}_out".format(i)].transpose(1, 2)
            ...
            passt_feature = passt_out_dict["layer{}_out".format(self.passt_feature_layer)]
            if not self.continual:
                passt_feature = passt_feature[:, :, 2:]
            passt_feature = passt_feature.transpose(1, 2)  # N,C,P->N,P,C
    ...
    def decoder_step(self, x, other_dict, encoder_win: bool, vectorize_transformers=False):
        # subsampling
        if not encoder_win:
        ...
        if self.decoder_name == 'gru':
            ...
            x = self.decoder(x, vectorize_transformers=vectorize_transformers)
        elif self.decoder_name == 'no':
            x = x
        return x

    def clean_state(self):
        self.patch_transformer.clean_state()
        self.decoder.clean_state()
        if self.slide_window_model is not None:
            self.slide_window_model.clean_state()
    ...
    def forward(self, input: torch.Tensor, encoder_win=False, mix_rate=0.5, win_param=[512, 49], temp_w=1):
        ...
        if not encoder_win:
            ...
            if self.slide_window_model is None:
                self.slide_window_model = PasstWithSlide(net=copy.deepcopy(self), win_param=win_param, continual=self.continual)
            x_local = self.slide_window_layer(self.slide_window_model(org_input))
            if passt_out_dict is None:
                return None

            x_global = self.f_pool(passt_out_dict)
            if x_global.size(1) > 1:
        ...
        # decoder
        x = self.decoder_step(x, other_dict, encoder_win=encoder_win, vectorize_transformers=input.size(-1) == 1)
    ...
    def forward_steps(self, *args, **kwargs):
