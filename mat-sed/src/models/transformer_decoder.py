...
class TransformerDecoder(co.CoModule, nn.Module):
    ...
    def forward(self, x, vectorize_transformers=False):
        B,T,C = x.shape
        x += self.pos_embed[:, :T, :]
    ...
    def forward_steps(self, x, vectorize_transformers=False):
        return self.forward(x, vectorize_transformers=vectorize_transformers)
...
class TransformerXLDecoder(co.CoModule, nn.Module):
    def __init__(self, input_dim, seq_len=1000 ,window_len=None, decoder_layer_num=2, continual=False) -> None:
        super(nn.Module, self).__init__()
        super(co.CoModule, self).__init__()
        self.config = TransformerDecoderConfig()
        ...
        self.encoder_blocks = nn.ModuleList([
            TransformerXL(input_dim, num_heads=self.config.num_heads, mlp_ratio=self.config.mlp_ratio, 
                          norm_layer=nn.LayerNorm, attn_drop=self.config.attn_drop, continual=continual) for i in range(decoder_layer_num)])
    ...
    def forward(self, x, pos_emb=None, vectorize_transformers=False):
        if pos_emb is None:
        ...
        for block in self.encoder_blocks:
            ...
            x = block(x, pos_emb, att_mask=att_mask, vectorize_transformers=vectorize_transformers)    #  # (T, N, C)
        
        x = x.permute(1, 0, 2)  # (T, N, C) ->(N, T, C)
        return x

    def forward_steps(self, x, vectorize_transformers=False):
        x, _ = self.encoder_pos(x)
        return self.forward(x, pos_emb=self.encoder_pos.pe, vectorize_transformers=vectorize_transformers)
