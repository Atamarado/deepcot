...
class BestModels:
    ...
    def update(self, net, ema_net, epoch, logger, val_metrics):
        ...
        val_metrics_std = val_metrics if ema_net is None else val_metrics[0]
        if val_metrics_std > self.stud_best_val_metric:
            self.stud_best_val_metric = val_metrics_std
            #self.stud_best_state_dict = train_cfg["net"].state_dict()
            self.stud_best_state_dict = copy.deepcopy(net.state_dict())

            stud_update = True
            # lr_reduc = 0
        if ema_net is not None and val_metrics[1] > self.tch_best_val_metric:
